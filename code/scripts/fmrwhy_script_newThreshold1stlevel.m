    ...
    for t = 1:numel(tasks)
        ...
        for r = 1:numel(runs)
            ...
            for e = 1:numel(echoes)
                ...
                if ~exist(run_dir_stats_new, 'dir')
...
%                spm_fn = fullfile(run_dir_stats, 'SPM.mat');
                ...
                for k = 1:numel(consess)
                    ...
                    conspec(k).threshdesc = 'none'; %'FWE'
                    conspec(k).thresh = 0.001; %0.05
                    conspec(k).extent = 0; %0
                    conspec(k).conjunction = 1;
                    conspec(k).mask.none = 1;
                end
                fmrwhy_batch_threshold1stlevel(run_dir_stats_new, conspec)
                % Rename files
                dt = datetime;
                y = num2str(year(dt));
                ...
                for k = 1:numel(consess)
                    src = fullfile(run_dir_stats_new, ['spm_' y m d '_' sprintf('%03d', k) '.jpg']);
                    dest = fullfile(run_dir_stats_new, ['statsresults_clusters_' sprintf('%03d', k) '.jpg']);
                    movefile(src, dest);
                end
                ...
                for k = 1:numel(consess)
                    tmap_fn = fullfile(run_dir_stats_new, ['spmT_' sprintf('%04d', k) '.nii']);
                    tmap_clusters_fn = fullfile(run_dir_stats_new, ['spmT_' sprintf('%04d', k) '_binary_clusters.nii']);
                    ...
                    saveAs_fn = fullfile(run_dir_stats_new, ['sub-' sub '_task-' task '_run-' run '_echo-' echo '_desc-' str '_noFWEthreshtmap.png']);

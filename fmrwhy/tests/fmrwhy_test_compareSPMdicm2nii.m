...
figure;
subplot(141);
imagesc(rot90(squeeze(img1(:, :, slice, 1))));
colorbar;
title('spm read vols');
subplot(142);
imagesc(rot90(squeeze(nii2.dat(:, :, slice, 1))));
colorbar;
title('spm nifti');
subplot(143);
imagesc(rot90(squeeze(p3.nii.img(:, :, slice, 1))));
colorbar;
title('nii viewer - dicm2nii');
subplot(144);
imagesc(rot90(squeeze(nii4.img(:, :, slice, 1))));
colorbar;
title('nii tool - dicm2nii');

%%
% Plot
figure;
subplot(121);
imagesc(rot90(squeeze(p3.nii.img(:, :, 20, 1))));
colorbar;
subplot(122);
imagesc(rot90(squeeze(p4.nii.img(:, :, 20))));
colorbar;
...
figure;
subplot(121);
imagesc(rot90(squeeze(nii_saved1.img(:, :, slice))));
colormap gray;
colorbar;
title('using original hdr');
subplot(122);
imagesc(rot90(squeeze(nii_saved2.img(:, :, slice))));
colormap gray;
colorbar;
title('4d to 3d conversion');

function [p, frm, rg, dim] = fmrwhy_util_readOrientNifti(fname, ask_code, reOri)
    ...
    if ischar(fname)
        p.nii = nii_tool('load', fname);
    else
        p.nii = fname;
        fname = p.nii.hdr.file_name;
    end
    ...
    if p.nii.hdr.scl_slope == 0
        p.nii.hdr.scl_slope = 1;
    end
...
function [R, frm] = nii_xform_mat(hdr, ask_code)
    ...
    if fs(1) == fs(2)
        fs = fs(1);
    end % sform if both are the same
    ...
    if numel(f) == 1 || nargin < 2 || isempty(ask_code) % only 1 avail or no ask_code
        ...
        frm = f(f == ask_code(1));
        if isempty(frm) && numel(ask_code) > 1
            frm = f(f == ask_code(2));
        end
        if isempty(frm) && any(f == 2)
            frm = 2;
        end % use confusing code 2
        if isempty(frm)
            frm = f(1);
        end % no match to ask_code, use sform
    end
...
function R = quat2R(hdr)
    ...
    if ~isreal(a)
        a = 0;
    end % avoid complex due to precision
...
function [R, perm, flp] = reorient(R, dim, leftHand)
    ...
    if any(ixyz(3) == ixyz(1:2))
        ixyz(3) = setdiff(1:3, ixyz(1:2));
    end
    [~, perm] = sort(ixyz);
    R(:, 1:3) = R(:, perm);
    flp = R([1 6 11]) < 0; % diag(R(1:3, 1:3))
    if nargin > 2 && leftHand
        flp(1) = ~flp(1);
    end
    rotM = diag([1 - flp * 2 1]);
...
function rg = get_range(nii, isLabel)
    ...
    ind = abs(img) > 50;
    if sum(ind) < numel(img) / 10
        ind = abs(img) > std(img) / 2;
    end
    im = img(ind);
    mu = mean(im);
    sd = std(im);
    ...
    if rg(1) < mi || isnan(rg(1))
        rg(1) = mi;
    end
    if rg(2) > ma || isnan(rg(2))
        rg(2) = ma;
    end
    if rg(1) == rg(2)
        rg(1) = mi;

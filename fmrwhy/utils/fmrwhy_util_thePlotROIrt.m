function fmrwhy_util_thePlotROIrt(functional_fn, mask_fn, roi_fn, task_info, trace_info, saveAs_fn)
    %
    ...
    % Ax2 - Task paradigm
    ax2 = subplot(7, 1, 1);
    im_task = imagesc(ax2, task_time_course');
    colormap(gray);
    ...
    plot(ax4, ROI_ts, 'LineWidth', 2, 'Color', colors_rgb{3});
    hold(ax4, 'on');
    NFB_ts = fmrwhy_util_scale(trace_info, -scale_f, scale_f);
    plot(ax4, NFB_ts, 'LineWidth', 2, 'Color', colors_rgb{4});
    xlim(ax4, [0 Nt]);
    ylim(ax4, [-2 2]);
    set(ax4, 'Xticklabel', []);
    set(ax4, 'Yticklabel', []);

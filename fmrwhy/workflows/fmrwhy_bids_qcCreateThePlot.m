function fmrwhy_bids_qcCreateThePlot(bids_dir, sub, task, options, varargin)
    %
    ...
    if options.theplot.include_physio
    ...
    x_gm = [0 0 1 1];
    y_gm = [0 gm_pos gm_pos 0];
    x_wm = [0 0 1 1];
    y_wm = [gm_pos gwm_pos gwm_pos gm_pos];
    x_csf = [0 0 1 1];
    y_csf = [gwm_pos gwmcsf_pos gwmcsf_pos gwm_pos];
    ...
    ax2.Position(1) = axpos(1);
    ax2.Position(3) = axpos(3);
    % Ax3 - Physiology
    ax3 = subplot(7, 1, 2);
    if options.theplot.include_physio
    ...
    ax3.Position(1) = axpos(1);
    ax3.Position(3) = axpos(3);
    ...
    ax4.Position(1) = axpos(1);
    ax4.Position(3) = axpos(3);
    ...
    xt = [2 2 2 2000 2000 2];
    yt = [7.2 4.7 2.2 3.4 1.3 0.5];
    for i = 1:6
        if i < 4
    ...
    % Save figure
    [filename, filepath] = fmrwhy_bids_constructFilename('func', 'sub', sub, 'ses', params.ses, 'task', task, 'acq', params.acq, 'rec', params.rec, 'run', params.run, 'echo', params.echo, 'desc', 'RO', 'ext', '_grayplot.png');
    theplot_fn = fullfile(options.qc_dir, filepath, filename);
    ...
    % Save figure
    [filename, filepath] = fmrwhy_bids_constructFilename('func', 'sub', sub, 'ses', params.ses, 'task', task, 'acq', params.acq, 'rec', params.rec, 'run', params.run, 'echo', params.echo, 'desc', 'GSO', 'ext', '_grayplot.png');
    theplot_fn = fullfile(options.qc_dir, filepath, filename);

...
functional_fn = spm_BIDS(BIDS, 'data', 'sub', sub ,'ses', ses, 'task', task, 'run', run, 'echo', echo, 'type','bold');
if numel(functional_fn) ~= 1
    warning(['Expected a cell array with a single file, found a cell array with ' num2str(numel(functional_fn)) ' files.'])
else
    functional_fn = functional_fn{1};
end

function confounds_tsv = fmrwhy_qc_generateMultRegr(bids_dir, sub, ses, task, run, echo)
...
% TODO: %order = 6; % 6 / 12 / 24 ==> MP, MP + MPdiff, MP + MPdiff + MPˆ2 + MPdiffˆ2
...
if ~exist(motion_fn, 'file')
    ...
    data_table = array2table(FD,'VariableNames', col_names);
    writetable(data_table, temp_txt_fn, 'Delimiter','\t');
...
if ~exist(tissue_regr_fn, 'file')
    ...
    % Get timeseries data
    rafunctional_4D = spm_readvols(spm_vol(rafunctional_fn));
...
if ~exist(confounds_fn, 'file')
    ...
    col_names = {'trans_x','trans_y','trans_z','rot_x','rot_y','rot_z','framewise_displacement','framewise_displacement_censor', 'white_matter', 'csf', 'global_signal', 'retroicor_c1','retroicor_c2','retroicor_c3','retroicor_c4','retroicor_c5','retroicor_c6','retroicor_r1','retroicor_r2','retroicor_r3','retroicor_r4','retroicor_r5','retroicor_r6','retroicor_r7','retroicor_r8','retroicor_cxr1','retroicor_cxr2','retroicor_cxr3','retroicor_cxr4','hrv','rvt'};
    data = [MP FD FD_outliers_regr WM_timeseries CSF_timeseries brain_timeseries phys_data.physio_regr];

...
T2star_thresholded(I_mask) = T2star(I_mask);
T2star_thresholded((T2star_thresholded(:) <= T2star_thresh_min)) = 1;
T2star_thresholded((T2star_thresholded(:) >= T2star_thresh_max)) = T2star_thresh_max;
S0_thresholded(I_mask) = S0(I_mask);
S0_thresholded(isnan(S0_thresholded(:))) = 0;
...
fmrwhy_util_saveNifti(saveAs_s0_fn, S0_3D_thresholded, template_fn, 'S0 map', 1)
